...
func main() {
	...
	dsn := "root:root@tcp(db:3306)/mysql?charset=utf8mb4&parseTime=True&loc=Local"
	db, err := gorm.Open(mysql.Open(dsn), &gorm.Config{})
	if err != nil {
		log.Error().Err(err)
		panic("Failed to open the MySQL database.")
	}
	...
	_ = app.Run(iris.Addr(fmt.Sprintf("%s:%d", "", 8000)))
}
